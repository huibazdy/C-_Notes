...
看到这里是不是觉得使用 `=` 就是拷贝初始化呢，其实还有以下几种情况也属于拷贝初始化：

* 对象作为实参传递给一个非引用类型形参；
* 函数返回一个非引用对象的类型；
* 花括号列表初始化一个数组中元素或一个聚合类中的成员。



标准库容器在初始化或调用 `insert` 或 `push` 成员时执行的是拷贝初始化，而使用 `emplace` 成员创建新元素时进行的是直接初始化。
...
在 C++11 标准下，除了拷贝构造函数、拷贝赋值运算符、析构函数外，还可以定义一个移动构造函数和一个移动赋值运算符。

需要哪些拷贝控制？判断步骤：

1. 是否需要自定义析构函数
2. 如果是，必然需要拷贝构造函数和拷贝赋值运算符
3. 使用 `=default` 显示地要求编译器生成合成
...
名为 `operator=` 的函数（运算符重载），也有返回类型和参数列表。参数是运算符的运算对象，左侧对象绑定到this参数，
...
构造函数初始化对象的非static数据成员；析构函数释放对象所使用的资源，并销毁对象的非 static 数据成员。
...
由于析构函数不接受参数，因此它不能被重载。一个给定类，只能有唯一的析构函数。

析构函数中首先执行函数体，然后成员按初始化的逆序被销毁。

销毁类类型的数据成员需要执行成员自己的析构函数。
