...
但我们使用该函数时：`total.combine(temp);`total的地址被绑定到隐式的this参数上，而rhs被绑定到temp上。

return语句解引用this指针来获得执行该函数的对象，换句话说，返回的是total的引用。

> 【复习：左值】（P121）
>
> C++表达式要么是右值要么是左值。
>
> 左值与右值继承于C语言，左值可以位于赋值语句左侧，右值则不能。
>
> C++中二者区别没那么简单。
>
> 左值表达式的求值结果是一个对象或一个函数，但以常量对象为代表的某些左值实际上不能作为赋值语句的左侧运算对象（与C语言的区别）。此外，某些表达式的求值结果是对象，但它们是右值而非左值。
>
> **归纳：**
>
> 当一个对象被用作右值时，用的是对象的值（内容）；当对象被用作左值时，用的是对象的身份（内存中的位置）。
>
> 函数的返回类型决定函数的调用是否为左值。调用一个返回引用的函数得到左值，其他类型得到右值。
